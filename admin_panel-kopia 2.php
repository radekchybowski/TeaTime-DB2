    ...
    if ($result = @$conn->query($_SESSION['sql_'.$table])) 
    {
        ...
        if ($result->num_rows > 0) 
        {
            ...
            if (isset($_POST['submitSearch']) && ($_POST['submitSearch'] != NULL)) {
                ...
                if ($table == 'herbaty')
                    $_SESSION['sql_herbaty'] = 'SELECT * FROM herbaty WHERE lower(' . $keys[1] . ') LIKE "%' . $search . '%" OR lower(' . $keys[2] . ') LIKE "%' . $search . '%" OR lower(' . $keys[3] . ') LIKE "%' . $search . '%" OR lower(' . $keys[4] . ') LIKE "%' . $search . '%" OR lower(' . $keys[5] . ') LIKE "%' . $search . '%" OR lower(' . $keys[6] . ') LIKE "%' . $search . '%" OR lower(' . $keys[7] . ') LIKE "%' . $search . '%" OR lower(' . $keys[8] . ') LIKE "%' . $search . '%" OR lower(' . $keys[9] . ') LIKE "%' . $search . '%";'; 
